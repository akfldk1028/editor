...
`platform/agent-runtimes/gitagent` is a repository-owned vendored snapshot of the
generic GitAgent executor. Its upstream identity is recorded in
`platform/agent-runtimes/gitagent/UPSTREAM.md`.
...
- `products/dwg` owns the independent DWG product and CAD runtime.
- Runtime source must not import PLAN Backend, PLANM, or DWG product code.

## Local verification

```powershell
npm --prefix platform/agent-runtimes/gitagent ci
npm --prefix platform/agent-runtimes/gitagent run build
npm --prefix platform/agent-runtimes/gitagent test
npm run test:agent
```
...
Backend always starts `products/plan/agents/planm/runtime/gitagent_host.mjs`. The host loads
