...
- `products/plan/agents/planm` remains a separate process boundary from Backend domain code.
- `platform/agent-runtimes/gitagent` remains a generic built runtime with no product imports.
  Its built discovery API is required by every deployed PLANM stage; no user
  activation switch or LLM provider is required.
- `products/dwg` is built from its complete source. Its public
