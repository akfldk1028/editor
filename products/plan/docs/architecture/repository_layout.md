...
platform/agent-runtimes/gitagent/  generic runtime; no product imports
products/dwg/                      independent DWG product
shared/contracts/plan-dwg/         versioned cross-product schemas
```

No `external/` or legacy root `agents/` directory exists. The generic runtime is
platform-owned and DWG is a sibling product rather than a PLAN agent.
...
- Backend modules never import PLANM or DWG product internals.
