...
*How to add or update a page under `docs/architecture/`.*

The pages in `docs/architecture/` are the canonical source of architectural rules. AI agents (Claude, Codex, Cursor, Gemini) read them on demand via `AGENTS.md`.

## Workflow

1. Pick a focused topic (one concept per page — layers, events, registry, …).
2. Create `docs/architecture/<slug>.md`. Slugs are short, kebab-case.
3. Add an entry to `docs/architecture/README.md` so the new page is discoverable.
...
- Reference real source files with a plain backtick path (e.g. `resources/lib/core/src/schema/base.ts`).
...
See `docs/architecture/README.md` for the current index.
