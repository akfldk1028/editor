...
승인은 수용 대안 2개를 요구한다. 현재 입력 외피는
`docs/architecture/input_envelope.md`의 48케이스로 실측되어 있으며, 직사각·오목·
비직교 플레이트의 전층 오피스와 상업층 혼합 케이스가 모두 그 문턱을 통과한다.
결과 수치는 커밋 시점 기록이므로 현재 상태를 말할 때는 sweep을 다시 실행한다.

모듈 경계를 넘는 평면 기하는 `GEOMETRY_DECIMALS` 그리드에 스냅해서 내보낸다
(`backend/engine/geometry/polygon.py`). 접촉 판정이 전부 정확한 Shapely
교집합이라 1나노미터 어긋난 벽은 벽이 아닌 것으로 읽힌다.

실 비례가 안 맞으면 게이트를 넓히지 말고 생성기가 치수를 다시 고르게 할 것.
게이트 완화는 RULES 4 위반이다.
