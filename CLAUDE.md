...
`docs/architecture/input_envelope.md`에 96케이스로 실측되어 있다. 직사각
플레이트는 전부 통과한다. L/U 오목 플레이트는 대안 전략이 바운딩 박스에
방을 깔기 때문에 앞단에서 거부된다 — `compose_structural_alternatives`가
폴리곤 안에 코어를 놓는 유일한 경로다.
