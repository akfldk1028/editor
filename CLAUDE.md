...
승인은 수용 대안 2개를 요구한다. 어떤 매스가 그 문턱을 넘는지는
`docs/architecture/input_envelope.md`에 48케이스로 실측되어 있다. 직사각
플레이트는 전부 통과하고, 오목·비직교 플레이트도 전층 오피스면 통과한다.
상업층이 섞이면 아직 형상별로 갈린다.
